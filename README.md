...
### "분양 예정 단지" — 공고 전 현장의 분양가 추정

**공고가 안 난 현장은 공공 API 에 없습니다.** 청약홈은 모집공고가 게시된 건만
공개합니다. 그래서 두 갈래로 나눠 다룹니다.

| 섹션 | 단지 출처 | 분양가 |
|---|---|---|
| 접수 중 공고 | 청약홈 API | API 확정값 |
| 접수 예정 공고 | 청약홈 API (공고 O, 접수 X) | API 확정값 |
| **분양 예정 현장** | **뉴스 (공고 X)** | **`src/lib/price-model.js` 로 추정** |

추정 산식:

```
평당 분양가 = 시군구·구 단위 실측 중앙값        ← 1순위
              (없으면) (택지비 + 건축비 × 브랜드계수) × (1 + 가산비율)
예상 분양가 = 평당 분양가 × 34평              ± 80분위 오차범위
```

**시세는 섞지 않습니다.** 원가식과 시세를 절반씩 섞으면 원가 추정인지 시세
반영인지 구분이 안 되고 검증도 불가능해서 걷어냈습니다.

### 캘리브레이션

`node scripts/calibrate.js` — 수집한 실제 분양가에서 지역별 평당가를 뽑아
`src/data/py-price.js` 를 생성합니다. 수집이 갱신되면 다시 돌리면 됩니다.

| 모델 | 평균 절대오차 | 중앙 | ±15% 안 |
|---|---|---|---|
| 손으로 정한 지역 표 (초기) | 28.4% | 21.6% | 31% |
| 광역 9버킷 역산 | 24.3% | 11.3% | 60% |
| 시군구 단위 실측 | 13.1% | 4.9% | 78% |
| **시군구 + 구 단위 실측** | **11.8%** | **4.4%** | **82%** |

표시 범위는 임의값이 아니라 측정된 **80분위(±14%)** 를 씁니다.
...
| 단지 | 시점 | 실제 | 추정 | 오차 |
|---|---|---|---|---|
| 과천 지식정보타운 | 2020 | 7.70억 | 8.40억 | +9.1% |
| 검단신도시 | 2024 | 5.20억 | 5.94억 | +14.3% |
| e편한세상 용인한숲시티 5단지 | 2015 | 2.77억 | 3.42억 | +23.5% |

과거 사례는 시점 보정 지수(건설공사비·지가)가 얹혀 오차가 커집니다.
특히 한숲시티는 6,800세대 초대형 완판형이라 같은 시군구 중앙값보다 훨씬
쌌습니다 — 단지 규모를 모델이 보지 않는 한계입니다.
