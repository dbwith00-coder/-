...
```
평당 분양가 = (평당 택지비 + 평당 건축비 × 브랜드계수) × (1 + 가산비율)
예상 분양가 = 평당 분양가 × 34평          ± 15% 범위 병기
```

- **택지비**: 지역별 표(강남 4,000만/평 ~ 지방 250만/평). 결과를 가장 크게 흔듭니다
- **건축비**: 표준 780만/평 × 브랜드계수(하이엔드 1.3 / 1군 1.1 / 일반 1.0)
- **가산비율**: 상한제 15% · 고가지역 28% · 수도권 25% · 지방 20%
- **지역**: 단지명에서 먼저 찾고, 없으면 기사 제목에서 찾습니다

### 백테스트

`node scripts/backtest.js` — 과거 분양 시점 조건으로 되돌려 실제 분양가와 대조합니다
(건설공사비지수·지가지수로 시점 보정).

| 단지 | 시점 | 실제 | 추정 | 오차 |
|---|---|---|---|---|
| e편한세상 용인한숲시티 5단지 | 2015 | 2.77억 | 3.03억 | **+9.2%** |
| 과천 지식정보타운 | 2020 | 7.70억 | 6.73억 | **−12.6%** |
| 검단신도시 | 2024 | 5.20억 | 5.14억 | **−1.1%** |

평균 절대오차 **7.6%**, 3건 모두 ±15% 범위 안.

> 파라미터는 실측이 아니라 시장 상황을 보고 정한 값이고 표본이 3건뿐입니다.
> 백테스트가 맞았다고 다른 단지에서도 맞는다는 뜻이 아닙니다.
